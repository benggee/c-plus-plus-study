...
int main(int argc, char *argv[]) {
	if (argc != 4) {
		printf("usage: ./reverse-proxy-out logfile conffile cmdport\n\n");
		printf("Sample: ./reverse-proxy-out /tmp/reverse-proxy-out.log ./forward-proxy.conf 4000\n\n");
	...
	if ((cmdlistensock = initserver(atoi(argv[3]))) < 0) {
		logfile.Write("initserver(%s) faile.\n", argv[3]);
		EXIT(-1);
	}
	...
	while(true) {
		...
		for (int i = 0; i < infds; i++) {
			...
			int j = 0;
			// 如果发生事件的是监听外网的listensock，表示有新的客户端连接上来
